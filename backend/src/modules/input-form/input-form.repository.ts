...
async function nextId(
  ...
  const r = await pool
    .request()
    .query(
      `SELECT ISNULL(MAX([${c.idColumn}]),0)+1 NextId FROM ${c.table}`,
    );
...
export async function insert(
  ...
  const r = await req.query(
    `INSERT INTO ${c.table} (${names}) VALUES (${vals}); SELECT TOP 1 * FROM ${c.table} WHERE [${c.childIdColumn}]=@${c.childIdColumn} ${
      isId
